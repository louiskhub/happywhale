...
def create_triplet_eval(model, train_ds, val_ds, train_df, val_df, new_whales_ds, new_whales_df, path_to_save,name):
    ...
    round = lambda x: np.round(x, 2)

    over_all_acc = round(np.mean(val_df["correctly_labeled"]))
    over_all_acc_species = round(np.mean(val_df["species_correctly_labeled"]))
    over_all_top5_acc = round(np.mean(val_df["correctly_labeled_k5"]))

    fig_info = name + f"Accuracies:  Individual-MAP: {over_all_acc}, Species-MAP: {over_all_acc_species}, k5-MAP - {over_all_top5_acc}"
    ...
    plt.title("2d UMAP Visualisation of Train Embeddings")
    ...
    ax.legend(loc='center left', bbox_to_anchor=(1, 0.5), title="Species")
    plt.figtext(0.5, 0.08, fig_info, wrap=True, horizontalalignment='center', fontsize=14)
    plt.title("2d UMAP Visualisation of Val Embeddings")
    ...
    mean_old = round(np.mean(vals[:, 0]))
    mean_new = round(np.mean(new_vals[:, 0]))
    p = (p9.ggplot(data=df,
